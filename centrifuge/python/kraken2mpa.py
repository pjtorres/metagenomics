...
#This code has been modified for or purposes
...
def main():
    ...
    main_lvls = ['R','K','D','P','C','O','F','G','S','Strains']
    for line in r_file:
...
#            print name , prev_lvl_num
	    curr_path.append(level_str)
            ...
            if (level_type == "x" and args.x_include) or (level_type != "x" and float(all_reads) != 0.00):
		...
		#if float(all_reads) != 0.00:
			#print all_reads, type(all_reads)
                o_file.write(level_str + "\t" + str(all_reads) + "\n")
                 
	 #Update
            curr_path.append(level_str)
            prev_lvl_num = level_num
    o_file.write('d_Unclassified_reads |p_Unclassified_reads |c_Unclassified_reads|o_Unclassified_reads|f_Unclassified_reads|g_Unclassified_reads|s_Unclassified_reads'+'\t'+unclassified_abun+'\n')
    o_file.close()
    r_file.close()
